...
- [ ] 15. Proxy Runtime / Data Plane Construct
  - [ ] 15.1 Test: `NetworkConstruct`와 `DataPlaneConstruct`는 VPC, subnet, security group, Aurora PostgreSQL Serverless v2, DB secret, optional RDS Proxy, DynamoDB cache table을 단일 stack 내부에 선언하고 cross-stack export 없이 참조된다. Code: `infra/constructs/network_construct.py`, `infra/constructs/data_plane_construct.py`를 추가한다. Verify: `pytest tests/iac/test_data_plane_construct.py -q && cdk synth`. Refactor after green: network/data props builder를 분리한다. Covers: R3.12, R11.2, R12.1, R12.2, R12.6.
